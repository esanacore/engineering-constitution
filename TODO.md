...
- [x] Redesign that diagram, which looked bad even after the mobile-rendering fix (clashing default mermaid theme, a feedback-loop edge that tangled under auto-layout, a wide layout that shrinks illegibly on a phone-width README column). Replaced with a muted custom theme, a one-directional (acyclic) flow, and a tall/narrow (`TD`) layout; documented the reasoning in `ARCHITECTURE.md` and `assets/diagrams/README.md` for future diagrams.
