...
- `scripts/check_wiki_links.sh`: Reference checker that verifies a wiki directory's internal integrity — flagging dangling `[[WikiLinks]]` (targets with no page) and orphan pages (linked by nothing). Resolves links by GitHub wiki rules; warns by default, `--strict` to fail.
- `scripts/measure_instruction_weight.sh`: On-demand meter (not a gate) that reports bytes, words, and estimated tokens for every document in the agent required-reading order — per document and in total, with an advisory `HEAVY` flag above ~8,000 estimated tokens. The one hard failure is a reading list naming a file that does not exist. Motivated by `sources/summaries/articles/the-harness-is-the-thing.md`: instruction weight has a per-session cost, and it should be measured rather than trimmed by taste.
