...
| OTS-104 | GitHub Actions: `actions/checkout` | v4.3.0 (SHA-pinned) | GitHub | Checks out the repository in every workflow here and in every workflow template | MIT | Medium (runs with the workflow token) | Pinned by full commit SHA in every workflow; Dependabot `github-actions` ecosystem proposes bumps | GitHub Advisory Database, last reviewed 2026-09-23 (no open advisories) | SHA-pinned, Dependabot-managed | Active |
