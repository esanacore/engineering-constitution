...
The framework also ships `scripts/check_wiki_freshness.sh`, a CI Enforcement tripwire (below) that flags a pull request which **adds or removes** source files without touching the wiki that catalogues them (see `docs/adr/0001-wiki-subsystem.md`). It is deliberately a *higher* bar than `check_doc_freshness.sh`, not a duplicate of it: doc-freshness asks "did any source change update README.md/CHANGELOG.md?", while this asks the narrower, wiki-shaped question "did the set of things this repository contains change — a file appeared or disappeared — without the wiki being updated?". Modifying existing files never trips it; only additions and deletions do, because a wiki is a high-altitude catalogue of capabilities, not a per-line mirror of the code. Its tests (`scripts/test_check_wiki_freshness.sh`) prove exactly that defining property — a modify-only diff exits 0 even under `--strict` — alongside the cases governance tooling must cover: an added file is flagged, a deleted file is flagged too (deletions are structural change, not only additions), test files and `docs/`/lockfile additions are correctly ignored, a custom `--wiki-dir` is honored, and a missing `--base`/`--head` reports a usage error. A checker whose ignore list silently did nothing, or that fired on every edit, would just be a noisier doc-freshness with no reason to exist.
...
- `constitution-wiki.yml` — the wiki-freshness tripwire (`check_wiki_freshness.sh`) on pull requests, a `links` job (`check_wiki_links.sh`) that checks link integrity on pull requests and pushes, and a publish-on-merge job that mirrors `wiki/` to the repository's built-in GitHub wiki so the rendered wiki never lags its reviewed source. See `docs/adr/0001-wiki-subsystem.md`.
