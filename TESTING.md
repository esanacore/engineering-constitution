...
- `constitution-wiki.yml` — the wiki-freshness tripwire (`check_wiki_freshness.sh`) on pull requests, plus a publish-on-merge job that mirrors `wiki/` to the repository's built-in GitHub wiki so the rendered wiki never lags its reviewed source. See `docs/adr/0001-wiki-subsystem.md`.
